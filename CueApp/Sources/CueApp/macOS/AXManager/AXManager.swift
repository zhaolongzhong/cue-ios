...
final class AXManager: ObservableObject {
    ...
    private let androidHandler: AndroidStudioHandler = AndroidStudioHandler()
    private let xcodeHandler: XcodeHandler = XcodeHandler()
    ...
    func loadTextAreas() {
        ...
        textAreaContentList = textAreas.enumerated().map { index, element in
            let (fileName, filePath): (String?, String?) = {
                switch ideType {
                case .androidStudio:
                    debugPrint("Using Android Studio handler")
                    return androidHandler.getAndroidStudioDocumentInfo(element, appElement: appElement)
                case .xcode:
                    debugPrint("Using Xcode handler")
                    return xcodeHandler.getXcodeDocumentInfo(element, appElement: appElement)
                case .other:
                    debugPrint("Using generic handler")
...
extension AXManager {
    func detectIDEType(_ runningApp: NSRunningApplication) -> IDEType {
        if androidHandler.isAndroidStudioOrJetBrainsIDE(runningApp) {
            return .androidStudio
        } else if xcodeHandler.isXcode(runningApp) {
            return .xcode
        } else {
            return .other
        }
    ...
    private func dumpHierarchyBasedOnIDEType(_ element: AXUIElement, appElement: AXUIElement, ideType: IDEType) {
        switch ideType {
        case .androidStudio:
            androidHandler.dumpAndroidStudioHierarchy(element)
        case .xcode:
            xcodeHandler.dumpXcodeHierarchy(element)
        case .other:
