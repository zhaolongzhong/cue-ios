...
actor EmailSummarizationService {
    ...
    // Process a specific category of emails for highlights
    func summarizeHighlights(emails: [CleanEmailMessage], category: EmailCategory, originalEmails: [String: GmailMessage]) async throws -> EmailHighlight? {
        guard emails.count >= 5 else {
            logger.debug("Not enough emails for \(category.displayName) highlights")
            throw SummarizationError.insufficientEmails
        }

        let emailStrings = emails.map { $0.toString(includeContent: true) }
        return try await summarizeHighlight(emailStrings, originalEmails: originalEmails)
    }
...
extension EmailSummarizationService {
    ...
    func summarizeHighlight(_ emails: [String], originalEmails: [String: GmailMessage]) async throws -> EmailHighlight {
        let cueClient = await CueClient()
        var messageParams: [CueChatMessage] = []
        ...
        if let highlight = handleHighlightResponse(from: updatedMessages.last, originalEmails: originalEmails) {
            return highlight
        } else {
            throw NSError(domain: "EmailSummarization", code: -1, userInfo: [NSLocalizedDescriptionKey: "Failed to parse email summaries"])
        }
    ...
    private func handleHighlightResponse(from message: CueChatMessage?, originalEmails: [String: GmailMessage]) -> EmailHighlight? {
        ...
        let validJSONString = parseToJSON(content)
            .trimmingCharacters(in: .whitespacesAndNewlines)

        return EmailHighlight.parse(from: validJSONString)
    }
