...
class AnthropicStreamingDelegate: Anthropic.StreamingDelegate {
    typealias ToolCallHandler = (String, [String: Any]) async -> String
    ...
    init(
        ...
        logger.debug("Initialized streaming delegate")
    }

    func didReceiveMessageStart(_ message: Anthropic.Message) async {
        logger.debug("Received message_start: \(message.id)")
    ...
    func didReceiveContentBlockStart(index: Int, contentBlock: Anthropic.ContentBlockStartEvent.ContentBlockStart) async {
        switch contentBlock {
        case .text:
            logger.debug("Content block start at index \(index): TEXT")
        case .toolUse(let toolUseBlock):
            logger.debug("Content block start at index \(index): TOOL_USE - \(toolUseBlock.name)")
            currentToolUseName = toolUseBlock.name
            currentToolUseId = toolUseBlock.id
            toolUseBuildUp[index] = ""
        case .thinking:
            logger.debug("Content block start at index \(index): THINKING")
    ...
    func didReceiveContentBlockDelta(index: Int, delta: Anthropic.ContentBlockDeltaEvent.DeltaContent) async {
        ...
        case .textDelta(let textDelta):
            ...
            await onEvent(.text(textDelta.text))
        ...
        case .inputJsonDelta(let jsonDelta):
            ...
            logger.debug("Tool JSON delta at index \(index): \(jsonDelta.partialJson)")
        ...
        case .thinkingDelta(let thinkingDelta):
            ...
            await onEvent(.thinking(thinkingDelta.thinking))

        case .signatureDelta(let sigDelta):
            logger.debug("Signature delta at index \(index): \(sigDelta.signature.prefix(20))...")
            thinkingSignatures[index] = sigDelta.signature
        }
    }

    func didReceiveContentBlockStop(index: Int) async {
        logger.debug("Content block stop at index \(index)")
        ...
        if let thinking = thinkingContent[index], !thinking.isEmpty,
           let signature = thinkingSignatures[index], !signature.isEmpty {
            logger.debug("Storing completed thinking block:")
            logger.debug("Thinking: \(thinking.prefix(50))...")
            logger.debug("Signature: \(signature.prefix(20))...")
            completedThinkingBlocks.append((content: thinking, signature: signature))

            // Create a thinking block
            // Note: We don't add it to contentBlocks yet, as we need to ensure proper ordering
        }
        ...
        if let toolInput = toolUseBuildUp[index], !toolInput.isEmpty,
           ...
           let toolId = currentToolUseId {
            ...
            logger.debug("Processing completed tool use at index \(index): \(toolName)")
            ...
            do {
                ...
                if let data = toolInput.data(using: .utf8) {
                    ...
                    await onEvent(.toolCall(toolName, inputParams))

                    // Execute the tool and get the result
                    let result = await onToolCall(toolName, inputParams)

                    // Create a tool result block for the final message
                ...
                logger.error("Error processing tool input: \(error)")
            }
    ...
    func didReceiveMessageDelta(stopReason: String?, stopSequence: String?, usage: Anthropic.Usage) async {
        logger.debug("Message delta - stop reason: \(stopReason ?? "none")")
    }
    ...
    func didReceiveMessageStop() async {
        ...
        if !messageBlocks.isEmpty {
            ...
            logger.debug("Created final message with \(messageBlocks.count) blocks")
        } else {
            logger.debug("No content blocks to create final message")
        }
    }
    ...
    func didCompleteWithError(_ error: Anthropic.Error) async {
        logger.error("Stream completed with error: \(error)")
    }
...
extension AnthropicStreamingDelegate {
    ...
    func waitForToolResults() async {
        ...
        if toolUseCount > 0 && toolResultCount < toolUseCount {
            logger.debug("Waiting for tool results to be processed: \(toolResultCount)/\(toolUseCount)")
            ...
            if toolResults.count < observedToolUses.count {
                logger.warning("Tool results may not be fully processed: \(self.toolResults.count)/\(self.observedToolUses.count)")
            }
        }
    }
    ...
    func hasCompleteToolResults() -> Bool {
        return toolResults.count >= observedToolUses.count
    }
}

extension AnthropicStreamingDelegate {
    private func logThinkingBlock(_ block: Anthropic.ThinkingBlock, prefix: String) {
        let shortThinking = block.thinking.prefix(50)
        let shortSignature = block.signature.prefix(20)
        logger.debug("\(prefix) Thinking: \(shortThinking)... Signature: \(shortSignature)...")
    }

    private func logMessageContentBlocks(_ messageBlocks: [Anthropic.ContentBlock]) {
        for (i, block) in messageBlocks.enumerated() {
            switch block {
            case .text(let textBlock):
                logger.debug("Block \(i): TEXT - \(textBlock.text.prefix(30))...")
            case .toolUse(let toolUseBlock):
                logger.debug("Block \(i): TOOL_USE - \(toolUseBlock.name)")
            case .thinking(let thinkingBlock):
                logger.debug("Block \(i): THINKING - \(thinkingBlock.thinking.prefix(30))...")
            }
        }
    }
}
