...
class EmailScreenViewModel: ObservableObject {
    ...
    private func trySummarizeHighlight(for category: EmailCategory) async {
        let summaries = emailSummaries.filter { $0.category == category }
        guard summaries.count >= 5 else {
            AppLog.log.debug("Not enough summaries for \(category.displayName)")
            return
        }

        let ids = summaries.map { $0.id }

        // Find all emails by ids
        let filteredEmails = ids.compactMap { id in
            originalEmails[id]
        }

        // Convert to clean emails
        let cleanEmails = filteredEmails.map { CleanEmailMessage(from: $0) }
        let emails = cleanEmails.map { $0.toString(includeContent: true)}
        do {
            let response = try await summarizationService.summarizeHighlight(emails, originalEmails: self.originalEmails)
            print("inx summarizeHighlight: \(response)")
        } catch {
            print("inx summarizeHighlight error: \(error)")
        }
    }
}
